	...
	<div id="main">
		...
		<div class="sub_contents_test_body">
			<form id="frmUserAnswer">
				<input type="hidden" name="book_seq"
					value="<?=$arr_output['book_info'][0]['seq']?>" /> <input
					type="hidden" name="test_seq"
					value="<?=$arr_output['test_info'][0]['seq']?>" /> <input
					type="hidden" name="published_seq"
				...
				<div class="uk-width-1-1 question_div"
					...
					<input type="hidden"
						...
						value="<?=$arrQuestionInfo['question_seq']?>" class="question_seq">
					<input type="hidden"
						name="user_answer[<?=$arrQuestionInfo['question_seq']?>]"
						id="user_answer_<?=$arrQuestionInfo['question_seq']?>"
						value="<?=$arrExample['seq']?>" class="question_seq"> <input
				...
				<!-- ######## -->
				<button type="button"
					onclick="objRegistration.submitQuestionAnswer();"
					class="pure-button pure-form_in btn-block submit_btn">
					답안지 전송 <i class="fa fa-arrow-right" aria-hidden="true"></i>
				</button>
				<!-- ######## -->
				<div class="h_dot">

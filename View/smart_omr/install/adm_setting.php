...
<div class="row">
	<div class="col-md-3 install_body etc_login">
		<ul>
			...
			<li><a href="javascript:void(0);" class="list-group-item login_facebook" onclick="checkSnsLogin($(this).attr('sns_type'),$(this).attr('event_type'),$(this));" ><i class="fa fa-facebook-official" aria-hidden="true"></i>페이스북 아이디로 로그인</a></li>
			<? }?>
			<? if($arr_output['$API_key']['naver']['client_id']!=''){ ?>
			<li>
			<a href="javascript:void(0);" onclick="$('#naver_id_login_anchor').click();" class="list-group-item login_naver"><span><img src="/smart_omr/_images/naver_logo.png" /></span>네이버 아이디로 로그인</a>
			<div id="naver_id_login" style="position:absolute;top:-1000px;"></div>
			</li>
			<? } ?>
			<? if($arr_output['$API_key']['kakao']['client_id']!=''){ ?>
			<li><a href="javascript:void(0);" onclick="loginWithKakao();" class="list-group-item login_kakao"><span><img src="/smart_omr/_images/kakao_logo.png" /></span>카카오톡 아이디로 로그인</a></li>
			<? } ?>
		</ul>
	</div>
</div>

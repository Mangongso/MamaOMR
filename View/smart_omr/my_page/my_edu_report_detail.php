		...
		<div class="container-fluid sub_container-fluid">
			...
			<div class="row content_body content_small_body">
				...
				<div class="sub_contents_body_box">
					<h4><?=$arrTest['subject']?><a
							class="uk-float-right uk-button uk-button-mini"
							href="/smart_omr/my_page/wrong_answer_notes.php?t=<?=md5($arrTest['test_seq'])?>">오답노트
					...
					<ul>
						...
						<li class="col-lg-4"><span><i class="fa fa-line-chart"
								aria-hidden="true"></i> 평균점수</span><?=round($arrTest['score_avarage'])?><small>점</small></li>
						<li>
					        ...
					        <li class="pure-menu-item">
					        	...
					        	<a
											href="/smart_omr/exercise_book/test_result.php?t=<?=md5($arrTest['test_seq'])?>&revision=<?=$arrMyRecordList['revision']?>&view=manager&ms=<?=$_GET['ms']?>"
											class="pure-menu-link"><i class="uk-icon-caret-right"></i><b><?=$intSubKey+1?>-</b><?=$arrMyRecordList['user_score']?><small>점</small></a>
					        	<? }else{ ?>
					        	<a
											href="/smart_omr/exercise_book/test_result.php?t=<?=md5($arrTest['test_seq'])?>&revision=<?=$arrMyRecordList['revision']?>"
											class="pure-menu-link"><i class="uk-icon-caret-right"></i><b><?=$intSubKey+1?>-</b><?=$arrMyRecordList['user_score']?><small>점</small></a>
					        	<? } ?>
					        </li>
					        <? } ?>
